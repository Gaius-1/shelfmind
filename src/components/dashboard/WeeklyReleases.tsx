...
import { TradeUpIcon, TradeDownIcon, Rocket02Icon } from '@hugeicons/core-free-icons'
...
export function WeeklyReleases({ weeklyJobs, totalJobs, thisMonthJobs, lastMonthJobs }: WeeklyReleasesProps) {
  // Compute real month-over-month growth
  const mom = React.useMemo(() => {
    if (lastMonthJobs === 0 && thisMonthJobs === 0) return { label: '0% M/M', isUp: true }
    if (lastMonthJobs === 0) return { label: '+100% M/M', isUp: true }
    const pct = Math.round(((thisMonthJobs - lastMonthJobs) / lastMonthJobs) * 100)
    return { label: `${pct >= 0 ? '+' : ''}${pct}% M/M`, isUp: pct >= 0 }
    ...
    <Frame spacing="xs" className="w-full h-full">
      <FramePanel className="w-full h-full p-6 flex flex-row items-center justify-between gap-6 overflow-hidden bg-card border-none shadow-none">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-sm text-muted-foreground font-medium">
              {weeklyJobs} {weeklyJobs === 1 ? 'job' : 'jobs'} processed this week.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-2 shrink-0 text-right">
          ...
          <Badge variant={mom.isUp ? 'success-light' : 'info-light'} className="rounded-md">
            <HugeiconsIcon icon={mom.isUp ? TradeUpIcon : TradeDownIcon} strokeWidth={2} className="size-3 mr-1" />
            {mom.label}
          </Badge>
        </div>
      </FramePanel>
