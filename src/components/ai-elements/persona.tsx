...
const PersonaWithModel = memo(
  ({ rive, source, children }: PersonaWithModelProps) => {
    ...
    const viewModelOptions = useMemo(() => ({ useDefault: true }), []);
    const viewModel = useViewModel(rive, viewModelOptions);

    const viewModelInstanceOptions = useMemo(() => ({
      rive,
      useDefault: true,
    }), [rive]);
    const viewModelInstance = useViewModelInstance(viewModel, viewModelInstanceOptions);
