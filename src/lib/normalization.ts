...
export function normalizeWeight(raw: string): string {
  ...
  const match = clean.match(/^([\d.,]+)\s*(g|kg|ml|l|grams|kilograms|liters|litres|millilitres|milliliters|pcs|pack|pk|oz|fl\.?\s*oz)?$/)
  if (match) {
    ...
    if (!isNaN(num)) {
      ...
      if (unit.startsWith('milliliter') || unit.startsWith('millilitre') || unit === 'ml') {
        return `${num}ml`
      }
...
export function normalizePackaging(raw: string): string {
  ...
  if (clean.includes('bottle') || clean.includes('hdpe') || clean.includes('pet')) return 'Bottle'
...
export function normalizeCountry(raw: string): string {
  ...
  // Title case default
  return raw.trim().replace(/\s+/g, ' ').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')
}
 ...
 */
export function normalizeField(column: ImdbColumnName, rawValue: string | null | undefined): string {
 ...
 */
export function normalizeRecord(record: Partial<Record<ImdbColumnName, string>>): Record<ImdbColumnName, string> {
